...
export class RecordingsService {
  ...
  async findOne(id: string): Promise<RecordingDocument> {
    return this.recordingAccess.findByIdWithPopulateOrThrow(id);
  }

  async addOperation(id: string, operationDto: AddOperationDto, userId: string): Promise<void> {
    await this.recordingAccess.getForOperation(id, userId);
  ...
  async completeRecording(id: string, finalCode: string, userId: string): Promise<RecordingDocument> {
    const recording = await this.recordingAccess.getForCompletion(id, userId);
  ...
  async remove(id: string, userId: string): Promise<void> {
    await this.recordingAccess.getForDeletion(id, userId);
  ...
  async getPlaybackData(id: string): Promise<{
    ...
    const recording = await this.recordingAccess.getForPlayback(id);
