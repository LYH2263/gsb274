...
export class RecordingsService {
  ...
  constructor(
    ...
    private readonly recordingValidator: RecordingValidator,
  ) {}
  ...
  async findOne(id: string): Promise<RecordingDocument> {
    return this.recordingValidator.findByIdOrThrow(id);
  }

  async addOperation(id: string, operationDto: AddOperationDto, userId: string): Promise<void> {
    await this.recordingValidator.findForAddOperation(id, userId);
  ...
  async completeRecording(id: string, finalCode: string, userId: string): Promise<RecordingDocument> {
    const recording = await this.recordingValidator.findForCompletion(id, userId);
  ...
  async remove(id: string, userId: string): Promise<void> {
    await this.recordingValidator.findForRemoval(id, userId);
  ...
  async getPlaybackData(id: string): Promise<{
    ...
    const recording = await this.recordingValidator.findForPlayback(id);
